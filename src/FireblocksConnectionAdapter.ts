...
export class FireblocksConnectionAdapter extends Connection {
  ...
  public static async create(
    ...
    try {
      const fireblocksSecretKey = FireblocksConnectionAdapter.parsePrivateKey(
  ...
  static parsePrivateKey(apiSecretPath: string): string {
    if (!apiSecretPath) {
      throw Error(
        `apiSecretPath is required in the fireblocks-web3-provider config`
      );
    }
    ...
    if (!apiSecretPath.trim().startsWith("-----BEGIN")) {
      ...
      return readFileSync(apiSecretPath, "utf8");
    } else {
